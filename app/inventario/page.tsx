...
type Item = { id: string; nome: string; categoria: string; quantidade_total: number; quantidade_disponivel: number; unidade: string; estado_conservacao: string; localizacao: string | null; numero_patrimonio: string | null; emprestimo_permitido: boolean; ativo: boolean; observacoes: string | null };
...
export default function InventarioPage() {
  ...
  function abrirItem() { setErro(""); setItemForm({ nome: "", categoria: "Esporte", quantidade_total: 1, unidade: "unidade", estado_conservacao: "Bom", localizacao: "", numero_patrimonio: "", emprestimo_permitido: true, observacoes: "" }); setModalItem(true); }
  ...
  async function salvarItem(e: React.FormEvent) {
    ...
    try {
      const r = await fetch("/api/inventario", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ acao: "criar_item", ...itemForm, quantidade_disponivel: itemForm.quantidade_total }) });
