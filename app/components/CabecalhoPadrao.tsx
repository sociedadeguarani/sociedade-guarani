...
export default function CabecalhoPadrao() {
  ...
  return <header className="sticky top-0 z-40 border-b border-[#dfe7e2] bg-white/95 shadow-sm backdrop-blur"><div className="flex min-h-[76px] items-center justify-between px-5 pl-16 lg:pl-6 lg:pl-[245px]"><div className="flex items-center gap-3"><div><div className="text-base font-extrabold text-[#005a3c] sm:text-lg">SOCIEDADE GUARANI</div><div className="text-xs text-gray-500">Sociedade Recreativa Guarani — S.R.G.</div></div></div><div className="flex items-center gap-3"><div className="hidden text-right sm:block"><div className="text-sm font-bold text-[#17382c]">{nome}</div><div className="text-xs text-[#005a3c]">{titulo}</div></div><button type="button" onClick={sair} className="rounded-xl border border-[#d5e0da] bg-white px-4 py-2 text-sm font-bold text-[#174133] hover:bg-[#f2f7f4]">Sair</button></div></div></header>;
}
