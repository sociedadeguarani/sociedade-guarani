...
const MENU = [
  ["Início", "⌂", "/painel"], ["Sócios", "♟", "/socios"],
  ["Dependentes", "♟", "/dependentes"], ["Reservas", "▣", "/reservas"],
  ["Eventos", "✣", "/eventos"], ["Convites", "▧", "/convites"],
  ["Avisos", "⚑", "/avisos"], ["Carteirinhas", "▤", "/carteirinhas"],
  ["Acessos", "⇥", "/acessos"], ["Financeiro", "▣", "/financeiro"],
  ["Relatórios", "▥", "/relatorios"], ["Inventário", "▱", "/inventario"],
  ["Usuários", "♙", "/usuarios"],
] as const;
...
export default function ConvitesPage() {
  ...
  return <main className="min-h-screen bg-[#f8faf9] text-[#173d2e]">
    <header className="sticky top-0 z-40 border-b border-[#dfe9e3] bg-white/95 shadow-sm backdrop-blur"><div className="flex h-20 items-center justify-between px-5 sm:px-7"><div className="flex items-center gap-4"><div className="flex h-12 w-12 items-center justify-center overflow-hidden rounded-xl bg-[#003d2b] p-1.5"><img src="/logo-guarani.png" alt="Sociedade Guarani" className="h-full w-full object-contain"/></div><div><h1 className="text-lg font-extrabold text-[#123c2b]">SOCIEDADE GUARANI</h1><p className="text-xs font-medium text-[#6b7d74]">Sociedade Recreativa Guarani — S.R.G.</p></div></div><span className="hidden text-sm text-gray-400 sm:block">Área Administrativa</span></div></header>
    <div className="flex min-h-[calc(100vh-80px)]"><aside className="hidden w-64 shrink-0 border-r border-[#dfe9e3] bg-[#f7faf8] p-3 md:block"><p className="mb-3 px-3 pt-2 text-[10px] font-extrabold uppercase tracking-[0.18em] text-[#91a099]">Menu principal</p><nav className="space-y-2">{MENU.map(([nome, icone, rota]) => <button key={nome} onClick={() => { if (rota !== "/convites") window.location.href = rota; }} className={`flex w-full items-center gap-3 rounded-xl px-4 py-3 text-left font-medium ${nome === "Convites" ? "bg-[#005a3c] text-white" : "text-[#50625a] hover:bg-[#e8f3ee] hover:text-[#005a3c]"}`}><span className="text-xl">{icone}</span>{nome}</button>)}</nav><div className="mt-10 rounded-2xl bg-[#f7edbd] p-4"><p className="text-xs font-bold text-[#705c00]">SOCIEDADE GUARANI</p><p className="mt-1 text-sm text-[#574900]">Sistema integrado de gestão</p></div></aside>
      <section className="min-w-0 flex-1 p-5 sm:p-7 lg:p-8"><div className="mb-7 flex flex-col justify-between gap-4 lg:flex-row lg:items-end"><div><p className="text-sm font-medium text-gray-500">Administração</p><h2 className="mt-1 text-3xl font-bold text-[#005a3c]">Convites</h2><p className="mt-1 text-gray-500">Controle de convidados, pagamentos e acesso à Sociedade.</p></div><button onClick={() => { setAbrir(true); setMensagem(""); }} className="rounded-xl bg-[#005a3c] px-5 py-3 font-bold text-white">＋ Novo convite</button></div>
