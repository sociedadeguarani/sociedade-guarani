...
export default function ConvitesPage() {
  ...
  useEffect(() => {
    async function carregar() {
      ...
      let listaSocios: Socio[] = [];
      if (token) {
        const r = await fetch("/api/socios", { headers: { Authorization: `Bearer ${token}` }, cache: "no-store" });
        if (r.ok) { const j = await r.json(); listaSocios = (j.socios || []).map((x: any) => ({ id: x.id, nome: x.nome, matricula: x.matricula ?? null })); }
      }
      setConvites((c || []) as Convite[]); setSocios(listaSocios); setContas((b || []) as Conta[]);
    }
    void carregar();
  }, []);

  async function enviarComprovante(convite: Convite, arquivo: File) {
    setEnviandoComprovante(convite.id); setMensagem("");
    try {
      const { data: sessao } = await supabase.auth.getSession();
      const token = sessao.session?.access_token || "";
      const fd = new FormData(); fd.append("origem_tipo", "convite"); fd.append("origem_id", convite.id); fd.append("arquivo", arquivo);
      const r = await fetch("/api/comprovantes/administracao", { method: "POST", headers: { Authorization: `Bearer ${token}` }, body: fd });
      const j = await r.json(); if (!r.ok) throw new Error(j.error || "Não foi possível enviar o comprovante.");
      setConvites(v => v.map(c => c.id === convite.id ? { ...c, comprovante_url: j.url, comprovante_status: "pendente" } as Convite : c));
      setMensagem("Comprovante enviado. O pagamento ficará aguardando aprovação da administração.");
    } catch (e) { setMensagem(e instanceof Error ? e.message : "Erro ao enviar comprovante."); }
    finally { setEnviandoComprovante(null); }
  }
