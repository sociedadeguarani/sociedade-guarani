...
export default function ConvitesPage() {
  ...
  async function salvar() {
    ...
    try {
      ...
      if (form.pagamento === "pix" && arquivoComprovante) {
        setEnviandoComprovante(data.id);
        const formData = new FormData();
        formData.append("origem_id", data.id);
        formData.append("arquivo", arquivoComprovante);
        const upload = await fetch("/api/comprovantes/administracao", { method: "POST", body: formData });
        const resultado = await upload.json().catch(() => ({}));
        if (!upload.ok) throw new Error(resultado.error || "Convite criado, mas não foi possível enviar o comprovante.");
        data.comprovante_url = resultado.url || null;
      }

      setConvites((lista) => [data as Convite, ...lista]);
      setAbrir(false); setMensagem(arquivoComprovante ? "Convite registrado e comprovante enviado. A administração foi notificada." : "Convite registrado com sucesso.");
    ...
    finally { setSalvando(false); setEnviandoComprovante(null); }
  }
