...
type Venda = {
  ...
  motivo_recusa: string | null; data_compra: string; eventos?: { titulo: string; data_inicio: string; local: string | null; imagem_url: string | null };
...
export default function EventosPage() {
  ...
  const [form, setForm] = useState<any>({ titulo: "", descricao: "", tipo: "evento", local: "Sociedade Recreativa Guarani", data_inicio: "", data_fim: "", imagem_url: "", link_externo: "", valor_ingresso: "", quantidade_disponivel: "", publicado: true, destaque: false });
  ...
  async function carregar() {
    ...
    try {
      ...
      setPerfil(j.perfil || ""); setEventos(j.eventos || []);
  ...
  function abrirNovo() { setArquivoImagem(null); setEditando(null); setForm({ titulo: "", descricao: "", tipo: "evento", local: "Sociedade Recreativa Guarani", data_inicio: "", data_fim: "", imagem_url: "", link_externo: "", valor_ingresso: "", quantidade_disponivel: "", publicado: true, destaque: false }); setModal(true); }
  function abrirEdicao(e: Evento) { setArquivoImagem(null); setEditando(e); setForm({ titulo: e.titulo, descricao: e.descricao || "", tipo: e.tipo || "evento", local: e.local || "", data_inicio: datetimeLocal(e.data_inicio), data_fim: datetimeLocal(e.data_fim), imagem_url: e.imagem_url || "", link_externo: e.link_externo || "", valor_ingresso: e.valor_ingresso ?? "", quantidade_disponivel: e.quantidade_disponivel ?? "", publicado: e.publicado, destaque: e.destaque }); setModal(true); }
  ...
  async function enviarComprovante(v: Venda) { if (!comprovante.trim()) return; setProcessando(true); try { const r = await api("/api/eventos/vendas/comprovante", { method: "PATCH", body: JSON.stringify({ id: v.id, comprovante_url: comprovante.trim() }) }); const j = await r.json(); if (!r.ok) throw new Error(j.error); setComprovante(""); await carregar(); const nv = (j.venda || null) as Venda | null; if (nv) setDetalhe(nv); } catch(e) { setErro(e instanceof Error ? e.message : "Erro ao enviar comprovante."); } finally { setProcessando(false); } }
