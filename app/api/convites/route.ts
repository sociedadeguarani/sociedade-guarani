...
async function bucket(db: any) {
  const atual = await db.storage.getBucket(BUCKET);
  if (!atual.error) return;
  const cr = await db.storage.createBucket(BUCKET, {
    public: true,
    fileSizeLimit: "8MB",
...
export async function POST(request: Request) {
  try {
    ...
    if (pagamento === "pix" && arquivo && !(arquivo instanceof File)) return NextResponse.json({ error: "Comprovante inválido." }, { status: 400 });
    if (arquivo instanceof File && (!TIPOS.includes(arquivo.type) || arquivo.size > 8 * 1024 * 1024)) {
    ...
    if (pagamento === "dinheiro") {
      const { data: conta, error } = await db.from("contas_bancarias").select("id,nome,banco").eq("id", conta_id).eq("ativo", true).maybeSingle();
      if (error) return NextResponse.json({ error: error.message }, { status: 500 });
    ...
    const { data: convite, error: ce } = await db.from("convites").insert({
      socio_id,
      nome_convidado: nome,
      documento_convidado: documento,
      cidade_convidado: cidade,
      data_inicio: inicio,
      data_fim: fim,
      tipo,
      valor,
      status: pagamento === "pix" ? "pendente" : "pago",
      forma_pagamento: pagamento,
      comprovante_url: null,
      comprovante_enviado_em: null,
      comprovante_status: pagamento === "pix" ? "nenhum" : "aprovado",
      motivo_recusa: null,
    }).select("*").single();
    ...
    if (pagamento === "dinheiro") {
      const { error: me } = await db.from("movimentacoes_financeiras").insert({
        ...
        observacoes: "Pagamento em dinheiro registrado pela administração.",
      });
      if (me) {
    ...
    if (arquivo instanceof File) {
      ...
      if (ne) return NextResponse.json({ ok: true, aviso: `Convite salvo, mas a notificação não foi criada: ${ne.message}`, convite: atualizado || convite, url: pub.publicUrl });
      return NextResponse.json({ ok: true, convite: atualizado || convite, url: pub.publicUrl });
    }

    return NextResponse.json({ ok: true, convite, aviso: "Convite PIX criado, mas nenhum comprovante foi anexado. Envie o comprovante antes da aprovação." });
  } catch (e) {
