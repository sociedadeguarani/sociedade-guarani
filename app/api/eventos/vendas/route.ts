...
async function gerarNumero(supabase: ReturnType<typeof getServiceClient>, eventoId: string, prefixo = "G") {
...
export async function GET(request: Request) {
  try {
    ...
    if (resultado.perfil === "associado") {
      query = query.eq("socio_id", resultado.usuario.socio_id || "00000000-0000-0000-0000-000000000000");
    }
...
export async function POST(request: Request) {
  try {
    ...
    const { data: evento, error: eventoError } = await resultado.supabase
      .from("eventos")
      .select("id,titulo,publicado,valor_ingresso,quantidade_disponivel,conta_bancaria_id,pix_copia_e_cola,imagem_url,local,data_inicio")
      .eq("id", evento_id)
      .single();
    ...
    if (evento.quantidade_disponivel != null) {
      const { data: vendas } = await resultado.supabase
        .from("eventos_vendas")
        .select("quantidade")
        .eq("evento_id", evento_id)
        .in("status", ["pendente", "aprovado"]);
      const vendido = (vendas || []).reduce((acc: number, item: { quantidade?: number }) => acc + Number(item.quantidade || 0), 0);
      if (vendido + quantidade > Number(evento.quantidade_disponivel)) {
        return NextResponse.json({ error: `Quantidade indisponível. Restam ${Math.max(0, Number(evento.quantidade_disponivel) - vendido)} ingresso(s).` }, { status: 409 });
      }
    }

    // O código é reservado no momento da compra e não muda quando o pagamento é aprovado.
    const { numero, prefixo } = await gerarNumero(resultado.supabase, evento_id, "G");
    ...
    const { data, error } = await resultado.supabase
      ...
      .select("*, eventos:evento_id(id,titulo,data_inicio,local,imagem_url,conta_bancaria_id,pix_copia_e_cola)")
      .single();
...
export async function PATCH(request: Request) {
  try {
    ...
    if (acao === "recusar") {
      ...
      const { data, error } = await resultado.supabase
        ...
        .update({
          ...
          aprovado_por: resultado.usuario.id,
    ...
    // Compatibilidade com compras antigas que ainda não tenham código.
    let updateCodigo: { prefixo?: string; numero?: number; codigo?: string; codigo_qr?: string } = {};
    if (!venda.codigo || !venda.numero) {
      const gerado = await gerarNumero(resultado.supabase, venda.evento_id, "G");
      const codigo = `G${String(gerado.numero).padStart(4, "0")}`;
      updateCodigo = { prefixo: "G", numero: gerado.numero, codigo, codigo_qr: codigo };
    }
