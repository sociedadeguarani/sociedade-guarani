...
export async function POST(request: Request) {
  ...
  try {
    ...
    const { data: acesso, error } = await supabase.from("acessos_sociedade").insert({ socio_id: socio.id, usuario_id: auth.usuario.id, resultado, local: body?.local || "Portaria" }).select("id,socio_id,usuario_id,entrada_em,local,resultado,observacao").single();
    if (error) throw error;
