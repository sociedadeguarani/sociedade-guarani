...
const [carregandoPreview, setCarregandoPreview] = useState(false);
  ...
  async function confirmarPagamento() {
    ...
    // A baixa manual sempre precisa indicar a conta que recebeu o valor.
    // Sem isso a mensalidade poderia ficar como "pago" sem gerar entrada no financeiro.
    if (!contaPagamentoId) {
      setMensagem("Selecione a conta bancária que recebeu o pagamento antes de confirmar a baixa.");
      return;
    }
    ...
    try {
      ...
      // Impede que a mesma baixa gere duas entradas no financeiro.
      const { data: movimentosExistentes, error: movimentosExistentesError } =
        await supabase
          .from("movimentacoes_financeiras")
          .select("id,origem_id")
      ...
      if (movimentosExistentesError) throw movimentosExistentesError;

      if ((movimentosExistentes || []).length > 0) {
        const idsDuplicados = new Set(
          (movimentosExistentes || []).map((movimento) => movimento.origem_id)
        );
        const nomesDuplicados = selecionadas
          .filter((m) => idsDuplicados.has(m.id))
          .map((m) => formatarCompetencia(m.competencia));

        throw new Error(
          `Já existe uma entrada financeira para ${nomesDuplicados.join(", ")}. A baixa não foi duplicada.`
        );
      }
